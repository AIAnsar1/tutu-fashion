...
class Banner(BaseModel, Base):
    ...
    url_path = Column(String(length=256), nullable=True)
    ...
    def seed(cls, fake, image_id, title, text_position="left"):
        banner = Banner(id=fake.uuid4(), title=title, image_id=image_id, text_position=text_position)
        return banner
